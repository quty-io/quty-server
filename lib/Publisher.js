...
class Publisher extends EventEmitter {
  ...
  /**
   * Private function that checks if we're connected before calling a function.
   * */
  [socketSend](event, data, fn) {
    if (!this.connected) {
      if (!this[config].buffer) return false;
      this[queue].push({
        event,
        data,
        fn
  ...
  send(channel, message, fn) {
    ...
    return this[socketSend](events.CLUSTER.CHANNEL_MESSAGE, {
      c: channel,
      m: message
    }, fn);
  }
  ...
  /**
   * Manually remove a client from a channel, if the client is connected to a channel.
   * @Arguments
   *  - channel - the channel we want to use
   *  - cid - the client id we want to remove.
   * */
  unsubscribeClient(channel, cid, fn) {
    if (typeof channel !== 'string' || !channel) return false;
    if (typeof cid !== 'string' || !cid) return false;
    return this[socketSend](events.CLUSTER.CLIENT_UNSUBSCRIBE, {
      c: channel,
      cid
    }, fn);
  }
  ...
  /**
   * Manually disconnect a connected client from any Quty server.
   * @Arguments
   *  - cid - the client id we want to disconnect.
   * */
  disconnectClient(cid, fn) {
    if (typeof cid !== 'string' || !cid) return false;
    return this[socketSend](events.CLUSTER.CLIENT_KICK, {
      cid
    }, fn);
  }
  ...
  flush() {
    ...
    for (let i = 0, len = items.length; i < len; i++) {
      let item = items[i];
      this[socketSend](item.event, item.data, item.fn);
    }
    return true;
  }
