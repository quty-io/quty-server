  ...
  async discover() {
    ...
    if (config.discovery.service) {
      try {
        ...
        ips.forEach((ip) => {
          discoveredNodes.push(`${ip}:${config.port}`);
        });
      } catch (e) {
    ...
    if (config.discovery.nodes instanceof Array) {
      for (let i = 0, len = config.discovery.nodes.length; i < len; i++) {
        ...
        if (typeof item === 'object' && item && item.ip) {
          discoveredNodes.push(`${item.ip}:${item.port || config.port}`);
        } else if (typeof item === 'string' && item) {
          discoveredNodes.push(item);
        }
      }
    }
    if (discoveredNodes.length === 0) return false;
  ...
  async addNode(address) {
    ...
    try {
      await clientObj.connect((resolve, reject) => {
        ...
        clientObj.on(events.CLUSTER.NODE_INFO, (data) => {
          clearTimeout(_t);
