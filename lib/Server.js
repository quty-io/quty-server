  ...
  async listen() {
    ...
    return new Promise((resolve, reject) => {
      this[http].listen(this[config].port, (err) => {
        if (err) {
          log.error(`(${this.name} server) cannot listen on port: ${this[config].port} [${this.id}]`);
          return reject(err);
        }
        log.info(`(${this.name} server) listening on port: ${this[config].port} [${this.id}]`);
        resolve();
        this.emit('listen');
      });
    });
...
function _handleWsConnection(socket) {
  ...
  socket.on('error', (e) => {
    log.trace(`Quty server: socket encountered error: ${e.message}`);
    cleanup();
    this.emit('disconnect', socket);
  });
  ...
  socket.isAlive = true;
  socket.on('pong', _handleHeartbeat);
}
